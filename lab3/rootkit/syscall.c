...
asmlinkage long hacked_unlinkat(int dfd, const char __user *filename, int flags){
	...
	if (strncmp(filename, must_be, strlen(must_be)) == 0) {
		return 1;
	}
...
static int __init hook_init( void  ) {
    ...
    orig_unlinkat = sys_call_table[__NR_unlink];
    sys_call_table[__NR_unlink] = hacked_unlinkat;
...
void __exit hook_exit( void  ) {
    pr_info("function: %s\n",__func__);
    sys_call_table[__NR_unlink] = orig_unlinkat;	
    return;
}
